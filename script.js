...
const agendaMoradores = {
    ...
    "Gate405": "11972079173", "Gate406": "11913374949","Gate407": "11916991214","Gate412": "11916270842",  "Gate413": "11946093516", "Gate414": "11998636282",
...
function enviarZap(item, tipo) {
    ...
    const tempoTotalMinutos = (hora * 60) + minutos;
    
    // Melhoria 3: Bom dia até 12:00, Boa tarde até 18:30
    let saudacao = "";
    if (tempoTotalMinutos < 720) {
        saudacao = "Bom dia";
    } else if (tempoTotalMinutos <= 1110) { // 18:30 = 1110 minutos
        saudacao = "Boa tarde";
    } else {
        saudacao = "Boa noite";
    }
...
function selecionarUnica(id) {
    ...
    if (item.status === 'Aguardando retirada') {
        ...
        // Melhoria 2: Rola automaticamente para o bloco de assinatura
...
// Melhoria 1: Botão Voltar ao Topo
...
function iniciarLeitor() {
    const scannerArea = document.getElementById('area-scanner');
    ...
    html5QrCode.start(
        ...
        config,
        (decodedText) => {
            const campoNF = document.getElementById('notaFiscal');
            if(campoNF) campoNF.value = decodedText;
            pararLeitor();
            campoNF.style.backgroundColor = "#e8f5e9";
            setTimeout(() => campoNF.style.backgroundColor = "", 1000);
        },
        (errorMessage) => {}
    ).catch(err => {
        alert("Não foi possível acessar a câmera. Digite manualmente.");
